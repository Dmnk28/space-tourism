        ...
        <main className='grid-container grid-container--destination z-1'>
            <div>
                ...
                <div className="destination-image" >
                    <Image src={destinations[showDestination].images.webp} width={445} height={445} layout="intrinsic"></Image>
                </div>
            </div>
            <div>
                <div className="tab-list underline-indicators flex ">
                    {destinations.map((element, index) => {
                        return (
                            <a key={element.name} className="uppercase ff-sans-condensed text-accent letter-spacing-2" onClick={handleDestination(index)}>{element.name}</a>
                        )
                    })}
                </div>
                <article>
